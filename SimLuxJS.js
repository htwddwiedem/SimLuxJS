...
class simLuxJS {
    ...
    // TW24 new data 
    perf_runTime = -1;  // for measurment of run time - value in ms 
    dbglv = 1;          // Debugglevel  0 = no info   1= minimal 5= maximal 
    ...
    async run(until=undefined) {
        ...
        //TW24
        let perf_endTime = performance.now();  // Execution  time in ms 
        this.perf_runTime = perf_endTime - perf_startTime;
        this.#logIfEnabled("Runtime=" + this.perf_runTime + " ms" ) ; 

        this.#logIfEnabled("Nothing is happening anymore -> the simulation ends.");
        return true;   // Original
        // return perf_runTime;  // TW24
    }

    // tw24
    async runSim(until=undefined) {
        console.log("Start Sim " ) ;
        let result = -111;
        await  this.run(until=undefined).then();
        result = 222; 
        console.log("End Sim "+ result) ;
    }
}
